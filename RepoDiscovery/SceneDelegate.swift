...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        favoritesNavigation.tabBarItem = UITabBarItem(title: NSLocalizedString("my_favorites", comment: ""), image: UIImage(systemName: "star"), tag: 1)
        
        // MARK: Settings
        
        let settingsViewController = SettingsViewController()
        let settingsNavigation = UINavigationController(rootViewController: settingsViewController)
        settingsNavigation.tabBarItem = UITabBarItem(title: NSLocalizedString("settings", comment: ""), image: UIImage(systemName: "gearshape"), tag: 2)
        
        
        // MARK: Tab Bar
        
        let tabBarController = UITabBarController()
        tabBarController.viewControllers = [discoveryNavigation,favoritesNavigation,settingsNavigation]
